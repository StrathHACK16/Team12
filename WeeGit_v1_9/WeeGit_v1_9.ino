...
boolean flashControl[3]= {false, false, false};
int cycles = 0;
...
void loop()
{
   ...
   else if (topLine.length() > 16 && ((millis() - timestamp) % 500 < 100)){
     cycles++;
     lcd.setCursor(0,0);
     if (cycles==(topLine + "    ").length())
       cycles = 0;
     lcd.print((topLine + "    ").substring(cycles, cycles + 16) + (topLine + "    ").substring(0,cycles));
     timestamp = millis();
...
boolean readSerial() {
  ...
  if (temp != ""){
    for (int i = 0;i<3;i++){
      if(temp.charAt(i) != UNCHANGED){
        ledControl[i] = temp.charAt(i) ==TRUE;
        flashControl[i] = false;
      }
      if(temp.charAt(i) == FLASH)
        flashControl[i] = true;
    }
  }  
  return true;
